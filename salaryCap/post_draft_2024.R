...
data <- read_csv("~/R Stuff/FantasyFootball 2.0/salaryCap/draft2024.csv")

optim <- data %>% 
  mutate(Cost = ifelse(Team...26 == "LB", Cost, Cost + 3)) %>% 
  select(Player, Team...2, Pos, FPTS, `Pos Rank`, `PAR Value`, `Inf Value`, Cost, `Estimated Cost`)
  

#Remove players
not_picking <- c("Puka Nacua", "Jacoby Brissett", "Deshaun Watson", "Russell Wilson", "CeeDee Lamb", "Jalen Hurts")
...
optim <- optim %>% 
  mutate(ones = 1,
         zeroes = 0,
         picked = ifelse(Player %in% players_picked, 1, 0)) %>% 
...
Salary_con <- 180
Player_con <- 8
Picked_con <- length(players_picked)
QB_con <- 2
RB_con <- 3
WR_con <- 5
TE_con <- 3
...
#Constraint Rhs
Const.rhs <- c(Salary_con, Player_con, Picked_con, QB_con, RB_con, WR_con, TE_con, min_QB_con, min_RB_con, min_WR_con, min_TE_con)
