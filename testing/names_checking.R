...
grouped_no_yahoo <- no_yahoo %>% 
  group_by(player) %>% 
  summarize(no_yahoo = n())

grouped_only_yahoo <- only_yahoo %>% 
  group_by(player) %>% 
  summarize(only_yahoo = n())


d <- full_join(grouped_combined, grouped_no_yahoo, by = c("player")) %>% 
  full_join(grouped_only_yahoo, by = c("player"))

t <- d %>% 
  filter(is.na(only_yahoo)) %>% 
  filter(total == no_yahoo)
