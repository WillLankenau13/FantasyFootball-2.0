...
starting_qb_ratings <- starting_qbs %>% 
  select(!team) %>% 
  left_join(QB_ratings, by = c("QB" = "player")) %>% 
  rename("player" = "QB")
...
player_predictions <- player_percents %>% 
  ...
  filter(pos != "QB" | player %in% starting_qbs$QB)
...
season_player_predictions[, 4:15] <- season_player_predictions[, 4:15]*(18 - upcoming_week)
...
t <- season_player_predictions %>% 
  select(player, team, pos, fpts_pred)
